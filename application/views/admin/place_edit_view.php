	...
	<div class='gform_body'>
		...
		<ul class='gform_fields top_label'>								
			...
			<li class='gfield'><label class='gfield_label'><?php echo $lang["road_plan"]?><span class='gfield_required'>*</span>:</label>
				<div class='ginput_container'>
					<input name='road_plan' language="no" type='file' value='<?php echo $entry['road_plan']?>' class='medium' tabindex='3'/>
				</div>
			</li>
			<li class='gfield'><label class='gfield_label'><?php echo $lang["phones"]?>:</label>
				<div class='ginput_container'>
					<input name='phones' language="no" type='text' value='<?php echo $entry['phones']?>' class='medium' tabindex='4'/>
				</div>
			</li>
			<li class='gfield'><label class='checkbox_label'><?php echo $lang["place_type"]?><span class='gfield_required'>*</span>:</label>
				<div class='ginput_container'>
					<select name="place_type">
						...
						{
						?>
						<option value="<?php echo $place_type['id']?>" <?php if ($place_type['id'] == $entry['place_type']) echo 'selected="selected"';?>><?php echo $place_type['name']?></option>
						<?php
						}
						?>
					</select>
					...
					<?php 						
						foreach ($languages as $value):
							...
							<div class="textarea" name='note' language="<?php echo $value['id'];?>">
								<textarea class='textarea small' tabindex='5' rows='10' cols='50'><?php echo $entry['note'][$value['id']]?></textarea>
							</div>
							<?php								
						endforeach;						
					?>
